...
WHERE b.intime + ((g.index_hour + 6) * interval '1 hour') <= b.outtime
...
SELECT
    ...
    CASE WHEN s.first_start IS NOT NULL AND s.first_start < s.index_time + interval '6 hours' THEN 1 ELSE 0 END AS label,
    CASE WHEN s.first_start IS NULL THEN NULL ELSE EXTRACT(EPOCH FROM (s.first_start - s.index_time)) / 3600.0 END AS lead_time_hours,
